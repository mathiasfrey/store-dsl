...
export default function PluginCard({ item, bgColor }) {
  ...
  const iconColor = pluginCardData.iconColor
    ? pluginCardData.iconColor.color
    : "white";

  let cssColors = CSS_COLOR_NAMES.map((color) => color.toLowerCase());
  let bgColorValidated = cssColors.includes(bgColor.toLowerCase())
    ? bgColor
    : "White";
  let iconColorValidated = cssColors.includes(iconColor.toLowerCase())
    ? iconColor
    : "DodgerBlue";
  ...
  return (
    <div className={Classes.PluginCardWrapper}>
      <div
        ...
        style={{ backgroundColor: `${bgColorValidated}` }}
      >
        <div className={Classes.Icon}>
          {getIconComponent({
            icon: icon,
            color: `${iconColorValidated}`,
            fontSize: "2.5rem",
            alignSelf: "left",
          })}
        </div>
